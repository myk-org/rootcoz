...
async def _get_job_token_usage_totals(
    ...
    """Run the shared per-job aggregate on an open connection."""
    cursor = await db.execute(
...
async def get_job_token_usage_totals(job_id: str) -> dict[str, Any] | None:
    ...
    async with _connect_db() as db:
        return await _get_job_token_usage_totals(db, job_id)


async def get_job_token_usage_details(
    job_id: str,
) -> tuple[dict[str, Any] | None, list[dict[str, Any]]]:
    """Return totals and calls from a single SQLite read snapshot."""
    async with _connect_db() as db:
        await db.execute("BEGIN")
        try:
            totals = await _get_job_token_usage_totals(db, job_id)
            if not totals:
                return None, []
            cursor = await db.execute(
                "SELECT * FROM ai_token_usage WHERE job_id = ? ORDER BY created_at ASC",
            ...
            return totals, [dict(row) for row in await cursor.fetchall()]
        finally:
            await db.rollback()
