...
def categorize_commit(
    ...
    default_category: str = "Other Changes:",
) -> str:
    ...
    title = commit["title"]
    try:
        prefix = title.split(":", 1)[
            0
        ].lower()  # Extract the prefix before the first colon
        return title_to_type_map.get(prefix, default_category)
    except IndexError:
        return default_category


def format_changelog_entry(change: dict[str, str], section: str) -> str:
    """Formats a single changelog entry."""
    title = (
        change["title"].split(":", 1)[1]
...
def main(from_tag: str, to_tag: str) -> str:
    ...
    for line in res.splitlines():
        ...
        if commit:
            category = categorize_commit(
                commit=commit, title_to_type_map=title_to_type_map
            )
    ...
    changelog += f"**Full Changelog**: https://github.com/RedHatQE/openshift-python-wrapper/compare/{from_tag}...{to_tag}"
